...
def load_model():
    ...
    MODEL_PATH = "outputs/models/final_model.h5"
...
def render_sidebar():
    with st.sidebar:
        ...
        show_gradcam = st.checkbox("🔥 Show Grad-CAM heatmap", value=True,
                                   help="Highlights which skin region triggered the prediction")
...
def compute_gradcam_inline(model, img_array):
    import tensorflow as tf
...
def main():
    inject_css()
    show_gradcam, show_topk = render_sidebar()
            ...
            if uploaded is not None:
                ...
                if model is None:
                    ...
                    if show_gradcam:
                        ...
                        st.caption("Highlights which region of the skin triggered the prediction")

                        try:
                            with st.spinner("Generating Grad-CAM..."):
                                heatmap = compute_gradcam_inline(model, img_array)
                                overlay = make_gradcam_overlay(heatmap, img_resized)

                            g1, g2 = st.columns(2)
                            with g1:
                                st.image(np.array(img_resized), caption="Original (224×224)",
